...
Process robot-side de plus, au même titre que `robot_agent.py` (base) et
`arm_agent.py` (bras) : lancé par `scripts/start_robot.sh` (flag `NO_MAST=1`
pour l'exclure), même contrat Zenoh `OPERATOR_IP` que ces deux-là (voir
`zenoh_config.load_robot_config` ci-dessous et son propre docstring pour
l'incident que ce contrat a corrigé). `--connect`/`--zenoh-config` restent
disponibles pour du test au banc, hors du stack complet (Arduino branché en
direct à une machine avec son propre zenohd local, sans `OPERATOR_IP`).
...
Deux sens de communication :
  ...
  SÉRIE → ZENOH (télémétrie remontée par l'Arduino)
    - robot/mast/state  (JSON {"position_mm": <float>, "fdc_min": <bool>,
        ...
        QoS "télémétrie" : DROP / BEST_EFFORT / DATA (cf. firmware/mast/README.md §5)
  ...
  ZENOH → SÉRIE (commandes envoyées par le PC opérateur, via web_server.py
  /ws/control {"type":"mast", ...} -- voir operator/web_server.py)
    - robot/mast/cmd    (abonnement) : traduit chaque requête en une ligne
        du protocole série du firmware `firmware/mast/src/main.cpp` :
          H · POS:<mm> · VEL:<mm/s> · STOP · UP_START/UP_STOP ·
          DOWN_START/DOWN_STOP · BRAKE:0/1 · FDC
...
Usage (stack complet, sur le PC robot) :
    OPERATOR_IP=192.168.15.111 python3 robot/mast_serial_bridge.py
    # port série par défaut : MAST_PORT (voir plus bas) ; override :
    MAST_PORT=/dev/serial/by-id/usb-xxxx OPERATOR_IP=192.168.15.111 \
        python3 robot/mast_serial_bridge.py

Usage (test au banc, routeur local, sans OPERATOR_IP) :
    python3 robot/mast_serial_bridge.py --port /dev/serial/by-id/usb-xxxx \
...
Le contrôle continu (monter/descendre depuis le front) utilise l'action
"velocity" réémise ≥10 Hz tant que le bouton est maintenu -- voir le
watchdog homme-mort firmware (300 ms sans nouveau VEL -> arrêt + frein),
décrit dans firmware/mast/README.md §5.
"""
...
from zenoh_config import load_robot_config  # noqa: E402
...
# Same by-id stable symlink pattern as ARM_PORT in arm_agent.py (NOT a bare
# /dev/ttyUSB0 -- USB-serial enumeration order isn't stable across reboots).
MAST_PORT = os.environ.get(
    "MAST_PORT",
...
def parse_args():
    ...
    p.add_argument(
        "--port",
        default=MAST_PORT,
        help=f"Port série de l'Arduino (défaut: MAST_PORT env var ou {MAST_PORT!r})",
    )
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Endpoint explicite du routeur zenohd à contacter (ex: "
            "tcp/localhost:7447), pour du test au banc SANS le stack complet. "
            "Omis (défaut) : utilise config/robot_zenoh.json5 + la variable "
            "d'environnement OPERATOR_IP, comme robot_agent.py/arm_agent.py "
            "(voir zenoh_config.load_robot_config). Ignoré si --zenoh-config "
            "est fourni."
        ),
    )
    p.add_argument(
        ...
        default=None,
        help=(
            "Fichier JSON5 de config Zenoh complet, pour un contrôle plus fin "
            "qu'un simple --connect."
        ),
    )
    p.add_argument(
        "--reconnect-delay",
...
def build_zenoh_config(args):
    """Construit la config Zenoh.

    Priorité : --zenoh-config (fichier explicite) > --connect (endpoint
    explicite, test au banc) > load_robot_config() (défaut : OPERATOR_IP +
    config/robot_zenoh.json5, même contrat que robot_agent.py/arm_agent.py).
    """
    ...
    if args.connect:
        ...
        return conf
    return load_robot_config("mast_serial_bridge")
